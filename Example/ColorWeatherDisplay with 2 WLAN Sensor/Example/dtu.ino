...
void DHTDataA() {
    ...
    if (httpCode > 0) {                   // Check if the request was successful
      ...
      DeserializationError error = deserializeJson(doc, payload);  // Parse JSON data
        if (Debug >1) {serializeJsonPretty(doc, Serial); } ; // nur Debug
        dhttempA = doc[0]["value"];
        dhthumA = doc[1]["value"];
        dhtpressA = doc[2]["value"]; //not in use, for future
        dhtwlanA = doc[3]["value"]; 
        dhtlightA = doc[4]["value"]; 
        dhtrainA =  doc[5]["value"]; 
        tempA = dhttempA;
        humA = dhthumA;
        wlanA = dhtwlanA;
        lightA = dhtlightA;
...
void DHTDataB() {
    ...
    if (httpCode > 0) {                   // Check if the request was successful
      ...
      DeserializationError error = deserializeJson(doc, payload);  // Parse JSON data
        if (Debug >1) {serializeJsonPretty(doc, Serial); } ; // nur Debug
        dhttempB = doc[0]["value"];
        dhthumB = doc[1]["value"];
        dhtwlanB = doc[2]["value"];
       } else Serial.println("Error retrieving DGT11");
    http.end();
   }
   return;
  ...
  sprite.print(solarleist);
    ...
    if (netz > 0) {
       ...
       sprite.print(netz);
        ...
        sprite.drawXBitmap(7,210,Haus1,40,40,TFT_YELLOW);
        ...
        //outside DHT11 
        sprite.setFont(&helvB12);
        sprite.drawXBitmap(197,210,Haus2,40,40,TFT_YELLOW);
        sprite.setCursor(250,210);
        sprite.print(dhttempB);
        //Serial.println(dhttemp);
        sprite.print(" 'C");
        sprite.setCursor(250,230);
        sprite.print(dhthumB);
        //serial.println(dhthumb);
        sprite .println(" % RH");
